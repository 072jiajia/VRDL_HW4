...
def activation(act_type='relu', inplace=True, slope=0.2):
    ...
    if act_type == 'relu':
        ...
        layer = nn.PReLU(num_parameters=1, init=slope)
    else:
...
def pad(pad_type, padding):
    # do padding after upsampling if needed
...
def sequential(*args):
    # custom sequential architecture generator
    if len(args) == 1:
        if isinstance(args[0], OrderedDict):
            raise NotImplementedError(
                '[ERROR] %s.sequential() does not support OrderedDict' % sys.modules[__name__])
        else:
            return args[0]
    modules = []
    for module in args:
...
def ConvBlock(in_channels, out_channels, kernel_size, stride=1,
              dilation=1, bias=True, valid_padding=True, padding=0,
...
class MeanShift(nn.Conv2d):
    # Kind of norm layer
    # subtract mean in the beginning of model
    # and add mean in the end of model
...
class FeedbackBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        del hr_features
        # leave out input x, i.e. lr_features[0]
...
class SRFBN(nn.Module):
    def __init__(self, in_channels, out_channels, num_features, num_steps, num_groups, act_type='prelu', norm_type=None):
        ...
        # basic block
        self.block = FeedbackBlock(num_features, num_groups,
                                   act_type, norm_type)
    ...
    def forward(self, x):
        ...
        inter_res = nn.functional.interpolate(x, scale_factor=3,
                                              mode='bilinear',
                                              align_corners=False)
        ...
        # return output of every timesteps
        return outs
